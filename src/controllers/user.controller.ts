...
const user_check = catchAsync(async (req: ExtendedRequest, res: Response) => {
    ...
    try{
         ...
         */
        const result = await services.user_check( data.data );
...
const user_logout = catchAsync(async (req: ExtendedRequest, res: Response) => {
    ...
    try{
         ...
         * Input Validation
         */
        const data = await validateInput(req.body, validate.user_logout);
        if (!data.status) {
            res.status(200).send( data );
            return;
        }
         ...
         * Authorization
         */
        let authData = authorize('user', 'user_logout', req);
        if (!authData.status) {
            DefaultResponse.error(res, '403');
            return;
        }
        ...
        data.data.hashToken = authData.data.hashToken;
        
        /**
         * @detail
         * Service function call
         */
        const result = await services.user_logout( data.data );
        res.status(200).send( result );
...
const user_delete = catchAsync(async (req: ExtendedRequest, res: Response) => {
    ...
    try{
         ...
         * Input Validation
         */
        const data = await validateInput(req.body, validate.user_delete);
        if (!data.status) {
            res.status(200).send( data );
            return;
        }
         ...
         * Authorization
         */
        let authData = authorize('user', 'user_delete', req);
        if (!authData.status) {
            DefaultResponse.error(res, '403');
            return;
        }
         ...
         * Service function call
         */
        const result = await services.user_delete( data.data );
        res.status(200).send( result );
