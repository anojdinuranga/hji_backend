...
const department_delete = catchAsync(async (req: ExtendedRequest, res: Response) => {
    ...
    try{
         ...
         * Input Validation
         */
        const data = await validateInput(req.body, validate.department_delete);
        if (!data.status) {
            res.status(200).send( data );
            return;
        }
         ...
         * Authorization
         */
        let authData = authorize('department', 'department_delete', req);
        if (!authData.status) {
            DefaultResponse.error(res, '403');
            return;
        }
         ...
         * Service function call
         */
        const result = await services.department_delete( data.data );
        res.status(200).send( result );
